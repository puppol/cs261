...
int bst_size(struct bst* bst) {
  ...
  if(bst == NULL)
    return 0;
  return bst_tree_size(bst->root);
}

/*
  This function recursively goes through the tree from the root node.

  Params:
    bst_node - the node that is currently being worked once
...
int bst_tree_size(struct bst_node* bst_node){
  if(bst_node == NULL)
    return 0;
  return bst_tree_size(bst_node->left) + 1 + bst_tree_size(bst_node->right);
}
...
int bst_height(struct bst* bst) {
  ...
  if(bst == NULL)
    return -1;
  return bst_tree_height(bst->root);
}

int bst_tree_height(struct bst_node* bst_node){
  if(bst_node == NULL)
    return -1;
  return 1 + getMax(bst_tree_height(bst_node->left), bst_tree_height(bst_node->right));
}
...
int bst_path_sum(int sum, struct bst* bst) {
  return bst_node_path_sum(sum, bst->root);
}

int bst_node_path_sum(int new_sum, struct bst_node * bst_node){
  int final;
  if(bst_node == NULL)
    return 0;
  else{
    ...
    new_sum = new_sum - bst_node->val;
    //printf("Sum: %d  Subbed: %d\n", (sum, subbed));
    if(new_sum == 0 && bst_node->left == NULL && bst_node->right == NULL)
      return 1;

    if(bst_node->left)
      final = bst_node_path_sum(new_sum, bst_node->left) || final ? 1 : 0;
    if(bst_node->right)
      final = bst_node_path_sum(new_sum, bst_node->right) || final ? 1 : 0;
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  bst_iter->vals = stack_create();
  if(bst && bst->root){
    create_bst_stack(bst->root, bst_iter->vals);
  }
...
void create_bst_stack(struct bst_node * bst_node, struct stack * bst_stack){
  if(bst_node == NULL)
    return;
  create_bst_stack(bst_node->right, bst_stack);
  stack_push(bst_stack, bst_node->val);
  create_bst_stack(bst_node->left, bst_stack);
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
  return (!stack_isempty(iter->vals));
}
